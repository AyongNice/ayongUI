...
import { UseDragDropRetunrn } from './index.d'; //分组 th组件
...
function Table({
  ...
  if (!groupHandleData.columns.length)
    return (
  ...
  /** 展开折叠记录状态 **/
  const [ayonEexpandedRowKeys, setAyonExpandedRowKeys] = useState<
    Array<number | string>
  >([]);
  ...
  _tableColumns.forEach((column: Column | ColumnGroup) => {
    if (column.type !== 'columnGroup') {
      ...
      if (sorter && typeof sorter === 'function') {
        _sortOrderMap[dataIndex] = defaultSortOrder
  ...
  const {
    ...
    handleDrop,
    handleDropData,
  }: UseDragDropRetunrn = useDragDrop({
    _tableColumns,
    data,
    draggable,
    onDragAfter,
  ...
  //排序状态
  const [sortOrderMap, stateSortOrderMap] = useState<{ [key: string]: string }>(
    _sortOrderMap,
  );
  ...
  const handleSort = (column: Column) => {
    ...
    if (sorter && typeof sorter === 'function') {
      stateSortOrderMap((prevState) => {
        ...
        map[dataIndex] = map[dataIndex]
          ? map[dataIndex] === 'ascend'
            ? 'descend'
            : 'ascend'
          : defaultSortOrder === 'ascend'
          ? 'descend'
  ...
  const getColSpan = (column: Column | ColumnGroup) => {
    return column.type === 'columnGroup'
      ? (column as ColumnGroup).children.length
      : void 1;
  };
  ...
  //手动计算 分配每列的宽度列表选项 用用于showOverflowTooltip 参数宽度限制
  useEffect(() => {
    const calculateColumnWidths = () => {
      ...
      if (table) {
        ...
        const averageWidth = width / columnCount;

        const widths = Array.from({ length: columnCount }, () => averageWidth);
  ...
  const onAllChange = (check) => {
    ...
    if (check) {
      tableData.map((_, index) => {
        const checkbox: CheckboxDataItem | null =
          rowSelection?.getCheckboxProps(_) || null;
  ...
  return (
    <table ref={tableRef} className={styleClassName}>
      <thead>
        <tr>
                ...
                <th style={{ width: ' 39px' }} rowSpan={colSpanSize}>
                  <Checkbox
                    ...
                    indeterminate={
                      ...
                      tableData.length > selectRowkeys.length
                    }
            ...
            <React.Fragment key={column.key}>
              ...
              >
                ...
                {column.sorter && (
                  ...
                  >
                    {sortOrderMap[column.dataIndex] &&
                    sortOrderMap[column.dataIndex] === 'default'
                      ? '↕'
                      : sortOrderMap[column.dataIndex] === 'ascend'
                      ? '↑'
                      : sortOrderMap[column.dataIndex] === 'descend'
                      ? '↓'
                      : ''}
                  </span>
                )}
