...
const DatePicker = React.forwardRef((props: CalendarProps, ref) => {
  ...
  const years = useMemo(() => {
    ...
    if (yearIndex > sourceData.length - 1)
  ...
  return (
    <div className={isRange ? '' : pickerStyle.warp}>
      {!isRange && (
        <main
          ...
          className={`${pickerStyle.main} ${
            selectDate ? pickerStyle.hoverMain : ''
          } ${disabled ? globe.disabled : pickerStyle.mainHover} ${className} `}
      ...
      <BaseCalendar
        className={`${isRange ? '' : pickerStyle.baseCalendarabsolute}  ${
          pickerStyle.baseCalendar
        } ${
          isDropdownVisible
            ? pickerStyle.baseCalendarShow
            : pickerStyle.baseCalendarNone
        }`}
        ref={childRef}
        {...props}
        selectedMode={picker}
        ...
        headerRender={({ curMonth, yearOptions, monthOptions }) => {
          return (
            <div
              className={pickerStyle.tshanHien}
              style={{ position: 'relative', marginBottom: ' 20px' }}
            >
              {showTime && (
                <>
                  <input
                    className={pickerStyle.input}
                    value={timeDate}
                    onChange={() => {}}
                  ...
                  <SHM
                    ...
                    className={`${pickerStyle.SHM} ${
                      showDownTime && pickerStyle.SHMShow
                    }`}
              ...
              <main className={pickerStyle.picker}>
                <div>
                  {['day', 'week', 'quarter', 'month', 'year'].includes(
                    picker,
                  ) && (
                    <Doubleleft
                  ...
                  {['day', 'week'].includes(picker) && (
                    <Left
                      onClick={() => childRef.current.prevMonth()}
                ...
                <div>
                  ...
                  {['month', 'quarter', 'day', 'week'].includes(picker) && (
                    <span> {curYear}年 </span>
                  )}
                  &nbsp;
                ...
                <div>
                  {['day', 'week'].includes(picker) && (
                    <Facright
                      onClick={() => childRef.current.nextMonth()}
                      className={`${pickerStyle.spacing}  ${pickerStyle.icon} `}
                    />
                  )}
                      ...
                      <main className="block-12">
                        {years.map((item, index) => {
                          return (
                            <div
                              key={index}
                              className={`block-chlid ${
                                item.value === selectDate ? 'onSelect' : ''
                              }`}
                              onClick={() => {
                        ...
                        {monthDate.map((item, index) => {
                          return (
                            <div
                              key={index}
                              className={`block-chlid ${
                                curYear + '年' + item.value === selectDate
                                  ? 'onSelect'
                                  : ''
                              }`}
                              onClick={() => {
          ...
          return typeof footerRender === 'function'
            ...
            : picker === 'day' && (
                <footer
                  ...
                  onClick={(event) => {
                    childRef.current.setDateSelected(
