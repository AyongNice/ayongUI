...
import { CalendarProps, DayItem } from ".././base-calendar/index.d";
import { Cendas, Doubleleft, Doubleright, Facright, Left, Wrong } from "../icon/icon.ts";
...
const DatePicker = React.forwardRef((props: CalendarProps, ref) => {
  ...
  useEffect(() => {
    if (value) {
      childRef.current?.handleItemClick(
        null,
        {
      ...
      childRef.current?.setCountYYYMMM({
  ...
  useEffect(() => {
    ...
    if (picker === "year") {
      ...
      for (let i = 0; i < yearDate.length; i++) {
        ...
        obj[yearDate[i].value.replace("年", "")] = arr.length - 1;
      }
      setYearIndex(() => obj[curYear]);
      setSourceData(arr);
    }

    if (picker === "quarter") {
      quarterDate = [{ value: "Q1" }, { value: "Q2" }, { value: "Q3" }, { value: "Q4" }];
    }
  }, []);

  const sshhmmref = useRef(null);
  useEffect(() => {
    const handleOutsideClick = (e) => {

      if (selectRef.current && !selectRef.current.contains(e.target) && !sshhmmref.current.contains(e.target)) {
  ...
  return (
    <div className={isRange ? "" : pickerStyle.warp}>
      {!isRange && (
        <main
          ref={selectRef}
          style={style}
        ...
        >
          <input className={pickerStyle.ssshhhmmInput} value={selectDate} disabled={disabled} onChange={() => {}} placeholder={placeholder[picker]} />
          <Wrong onClick={onWrong} className={pickerStyle.pickerWrong} />
          <Cendas className={pickerStyle.pickerCollapse} />
        </main>
      )}
      <BaseCalendar
        className={`${isRange ? "" : pickerStyle.baseCalendarabsolute}  ${pickerStyle.baseCalendar} ${
          isDropdownVisible ? pickerStyle.baseCalendarShow : pickerStyle.baseCalendarNone
        }`}
        ref={childRef}
        {...props}
        selectedMode={picker}
        ...
        headerRender={({ curMonth, yearOptions, monthOptions }) => {
          return (
            <div ref={sshhmmref} className={pickerStyle.tshanHien} style={{ position: "relative", marginBottom: " 20px" }}>
              {showTime && (
                <>
                  <input className={pickerStyle.input} value={timeDate} onChange={() => {}} onFocus={() => setShowDownTime(true)} placeholder="选择几点钟" />
                  <SHM
              ...
              <main className={pickerStyle.picker}>
                <div>
                  {["day", "week", "quarter", "month", "year"].includes(picker) && <Doubleleft onClick={togglePrevYear} className={pickerStyle.icon} />}
                  {["day", "week"].includes(picker) && <Left onClick={() => childRef.current.prevMonth()} className={`${pickerStyle.spacing}  ${pickerStyle.icon} `} />}
                </div>
                <div>
                ...
                <div>
                  {["day", "week"].includes(picker) && <Facright onClick={() => childRef.current.nextMonth()} className={`${pickerStyle.spacing}  ${pickerStyle.icon} `} />}

                  {["day", "week", "quarter", "month", "year"].includes(picker) && <Doubleright onClick={toggleNextYear} className={pickerStyle.icon} />}
                </div>
              </main>
            </div>
          );
        ...
        dateRender={
          ["quarter", "month", "year"].includes(picker)
            ? (item) => {
                return (
                  <React.Fragment>
                    {"year" === picker && (
                      <main className="block-12">
                        ...
                        {monthDate.map((item, index) => {
                          return (
                            <div
                              ...
                              onClick={() => {
                                _onChange({
                                  comprehensiveStr: curYear + "年" + item.value,
                                });
                              }}
                            >
                              {item.value}
